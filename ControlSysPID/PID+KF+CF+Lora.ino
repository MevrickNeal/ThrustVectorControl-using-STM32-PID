/*
 ...
 Description:  This is the core control loop for the TVC system.
               It reads orientation from the MPU6050, altitude from
               the BME280, and sends a full telemetry packet via LoRa.
               **UPDATE: Expanded LoRa telemetry packet with all flight data.**
...
float vertical_velocity = 0; // Estimated vertical velocity in m/s
...
void loop() {
  ...
  if (millis() - last_telemetry_send > 100) {
    ...
    // --- Collect all data points for telemetry ---
    ...
    // Integrate acceleration to get velocity (simple estimation)
