/*
 ...
 Description:  This is the core control loop for the TVC system.
               ...
               **UPDATE: Now includes a Complementary Filter for smooth sensor data.**
...
double angleX, angleY;               // The **filtered** angle from the sensor
...
float pitch_acc, roll_acc; // Angles calculated from accelerometer
unsigned long timer;       // Timer for calculating loop time
float time_step;           // Time duration of each loop in seconds
...
void setup() {
  ...
  // NEW: Set up the gyroscope range
...
void loop() {
  ...
  // --- NEW: Complementary Filter Implementation ---
  
  // Calculate the time step for this loop iteration
  time_step = (float)(micros() - timer) / 1000000.0; // Convert microseconds to seconds
  timer = micros(); // Reset timer for the next loop

  // Calculate raw angles from the accelerometer (stable but noisy)
  roll_acc = atan2(a.acceleration.y, a.acceleration.z) * 180.0 / PI;
  pitch_acc = atan2(-a.acceleration.x, sqrt(a.acceleration.y * a.acceleration.y + a.acceleration.z * a.acceleration.z)) * 180.0 / PI;

  // Integrate the gyroscope data (fast but drifts over time)
  // g.gyro.x is in radians/s, so we convert to degrees/s
  angleX = angleX + (g.gyro.x * 180.0 / PI) * time_step;
  angleY = angleY + (g.gyro.y * 180.0 / PI) * time_step;

  // Combine the two readings. 98% from the fast gyro, 2% from the stable accelerometer.
  // This corrects the gyro's drift over time.
  angleX = 0.98 * angleX + 0.02 * roll_acc;
  angleY = 0.98 * angleY + 0.02 * pitch_acc;

  // --- Compute the PID correction (using the new smooth angles) ---
  pidX.Compute();
  pidY.Compute();
  ...
  // The delay is no longer needed as the loop timing is handled by the filter
  // delay(20); 
}
