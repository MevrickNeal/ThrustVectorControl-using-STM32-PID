/*
 ...
 Description:  This is the core control loop for the TVC system.
               ...
               **UPDATE: Added SD Card data logging. halts the system if thesd card is missing**
...
#include <SPI.h>
#include <LoRa.h>
#include <SD.h> // NEW: For the SD Card
...
File flightLogFile; // NEW: File object for our log file

// --- Pin Definitions ---
const int SD_CS_PIN = PB0; // NEW: SD Card Chip Select pin
...
void loop() {
  ...
  if (millis() - last_telemetry_send > 100) {
    ...
    // --- NEW: Write the same packet to the SD card ---
    flightLogFile = SD.open("flightlog.csv", FILE_WRITE);
    if (flightLogFile) {
